...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tambahTagihan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tambah-tagihans.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="username_id">{{ trans('cruds.tambahTagihan.fields.username') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.tambahTagihan.fields.username_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tagihan_status') ? 'is-invalid' : '' }}" name="tagihan_status" id="tagihan_status">
                    ...
                    @foreach(App\Models\TambahTagihan::TAGIHAN_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tagihan_status', 'aktif') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
